...
export class CollectionManager {
  ...
  public async search<R = AnyObject>(
    query: SearchParams,
  ...
  public NLPSearch<R = AnyObject>(
    params: NLPSearchParams,
  ): Promise<NLPSearchResult<R>[]> {
  ...
  private parseGeneratedQueries(queries: any): GeneratedQuery[] {
    if (!Array.isArray(queries)) {
      console.warn(
        "Expected array for GENERATED_QUERIES, got:",
        typeof queries,
      );
      return [];
    }
    ...
    return queries.map((data: any, index: number): GeneratedQuery => {
      try {
        ...
        console.warn(
          `Failed to parse generated query at index ${index}:`,
          error,
        );
        return {
          index,
  ...
  public createAnswerSession(
    config?: CreateAnswerSessionConfig,
  ): AnswerSession {
    if (!this.readAPIKey) {
  ...
  public insertSegment(
    segment: InsertSegmentBody,
  ): Promise<InsertSegmentResponse> {
  ...
  public insertTrigger(
    trigger: InsertTriggerBody,
  ): Promise<InsertTriggerResponse> {
  ...
  public insertTool(tool: InsertToolBody) {
    ...
    switch (true) {
      ...
      default:
        throw new Error(
          "Invalid parameters type. Must be string, object or ZodType",
        );
    }
  ...
  public async executeTools<Response = AnyObject>(
    ...
    const response = await this.oramaInterface.request<
      ExecuteToolsParsedResponse<string>
    >({
    ...
    if (response.results) {
      return {
        results: response.results.map(
          (result): ExecuteToolsResult<Response> => {
            ...
            if ("functionParameters" in result) {
              return {
                functionParameters: {
                  ...
                  result: JSON.parse(
                    result.functionParameters.result,
                  ) as Response,
                },
              };
            }
